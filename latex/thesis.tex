...
\begin{center}
\begin{tikzpicture}[node distance = 1.5cm]d
    % Place nodes
    \node [pierwsza] (jeden) {Generowanie mapy};
    \node [pierwsza, below of=jeden] (dwa) {Generowanie agentów};
    \node [druga, below of=dwa] (trzy) {Symulowanie łańcucha logistycznego (dostaw)};
    \node [druga, below of=trzy] (cztery) {Symulowanie decyzji klientów (zakupów)};
    \node [trzecia, below of=cztery] (piec) {Zastosowanie algorytmu optymalizacyjnego)};
    \node [trzecia, below of=piec] (szesc) {Sprawdzenie wpływu algorytmu na wyniki firmy)};

\path [line,dashed] (jeden) -- (dwa);
\path [line,dashed] (dwa) -- (trzy);
\path [line,dashed] (trzy) -- (cztery);
\path [line,dashed] (cztery) -- (piec);
\path [line,dashed] (piec) -- (szesc);
\end{tikzpicture}
...
\begin{tikzpicture}[node distance = 1.5cm]d
    % Place nodes
    \node [pierwsza] (jeden) {Stwórz pustą macierz o wymiarach x,y};
    \node [pierwsza, below of=jeden] (dwa) {Wylosuj punkt startowy};
    \node [druga, below of=dwa] (trzy) {Czy punkt spełnia kryteria?};
    \node [pierwsza, below of=trzy] (cztery) {Umieść drogę w tym punkcie};
    \node [pierwsza, right of=trzy] (piec) {Wylosuj kolejny punkt w sąsiedztwie};
    \node [pierwsza, below of=cztery] (szesc) {Kontynuuj aż ilość dróg będzie równa założeniom)};
    \node [pierwsza, below of=szesc] (siedem) {Dla kazdego punktu na mapie - czy sąsiaduje z drogą?)};
    \node [pierwsza, right of=siedem] (dziewiec) {Wylosuj lokalizację ze zbioru $(dom,praca,przestrzen komercyjna)$};
\path [line,dashed] (jeden) -- (dwa);
\path [line,dashed] (dwa) -- (trzy);
\path [line,dashed] (trzy) -- (cztery);
\path [line,dashed] (cztery) -- (piec);
